...
def build_insight_prompt(
    ...
    "   - Nếu ít nhất một mốc trong câu hỏi nằm trong cửa sổ dữ liệu → trạng thái IN_RANGE.\n"
    "   - Nếu tất cả mốc đều nằm ngoài cửa sổ dữ liệu → trạng thái OUT_OF_RANGE.\n\n"

    "2. Xử lý theo trạng thái dữ liệu:\n"
    "   - Nếu data_rows trống hoặc chỉ gồm min/max-window và trạng thái là OUT_OF_RANGE → "
    f"trả về đúng câu: 'Không có dữ liệu cho mốc đã hỏi, dữ liệu chỉ có từ {window_txt}.'\n"
    "   - Nếu data_rows trống nhưng trạng thái là IN_RANGE (ví dụ: lọc 'lỗ' nhưng không có nhóm nào lỗ) → "
    f"trả về đúng câu: 'Dữ liệu không có thông tin.'\n"
    "   - Nếu có dữ liệu hợp lệ → viết insight ngắn, nêu kết quả chính (doanh thu, lợi nhuận, số lượng, v.v.) "
    "và xu hướng nếu có. Không dùng từ hoa mỹ hoặc suy diễn.\n\n"

    "**Gợi ý diễn giải từ khóa:**\n"
    " - 'Lỗ' → profit < 0\n"
    " - 'Lãi' hoặc 'lợi nhuận' → profit > 0\n"
    " - 'Doanh thu' → sales (giá trị không âm)\n\n"
